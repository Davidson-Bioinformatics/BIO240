...
First, we need to start by checking baseline assumptions before fitting a linear model. A linear model depends on the response variable being normally distributed. 

Question 5: Determine if the response variable is normally distributed through any graphing or statistical means.

Once this condition is satisfied we can construct a linear model and then check the residuals to ensure that they are normally distributed as well. To construct the linear model, use the function lm() in R. We give the names of the two variables, and then tell R where to find the dataframe with the data = option. This model should be saved to a variable (like in anova) and then we can look at the results using the summary() function.
...
Now that we have our linear model, it is good to investigate the residuals. We can pull the residuals out of our linear model object and save them to a new variable. This will be a vector (list) of the residuals. 

```{r}
wolf.residuals <- wolf.lm$residuals
```

Question 8: Create a residual plot of the residuals for this data. Do the residuals satisfy the conditions we discussed in class? Create any additional graphs necessary to check the assumptions of the residuals. 
